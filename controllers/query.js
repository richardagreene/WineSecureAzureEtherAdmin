...
function QueryController(request, response, next) {
    ...
    return new Promise((resolve, reject) => {
        getNode().then((ip) => {
            ...
            contractInstance.wineDetail((err, result) => {
                ...
                var data = { owner: contractInstance.owner, address: address, name: result[0], vineyard: result[1], grapeVariety: result[2], colour: result[3], alcoholLevel: result[4], status: result[5] };
                resolve(data);
            });
        });
    });
